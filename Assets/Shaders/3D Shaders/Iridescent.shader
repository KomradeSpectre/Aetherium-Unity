Shader "Toon/Iridescent Mask" {
	Properties{
		...
		_WorldScale ("Noise Worldscale", Range (.002, 100)) = 1 // noise scale
...
#pragma lighting ToonRamp exclude_path:prepass
	...
	sampler2D _EmissionMap; //emission
	float4 _Color;
	...
	void surf(Input IN, inout SurfaceOutput o) {
		...
		half4 emission = tex2D(_EmissionMap, IN.uv_EmissionMap) * _EmissionColor; //emission
		c.rgb += emission.rgb;
